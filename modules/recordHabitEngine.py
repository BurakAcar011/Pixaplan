def record(username, password, graphID, month, day, year, quantity):
    ...
    request = requests.post(f"https://pixe.la/v1/users/{username}/graphs/{graphID}", json=params, headers=header)
    jsonrequest = request.json()

    # print(jsonrequest)
    if jsonrequest.get("isSuccess", True):
        messagebox.showinfo("Success", "Habit recorded successfully")
    else:
