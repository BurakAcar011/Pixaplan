...
import modules.recordHabitEngine as recordHabitEngine
...
class recordHabitUI():
    def showYourself():
        ...
        window = Tk()
        window.title("Record Habit")
        window.config(bg=THEMECOLOR, padx=10, pady=10)
        grandTitle = Label(window, text="Add Recording", font=("Arial", 24), bg=THEMECOLOR, fg=FOREGROUND)
        ...
        def recordfunct():
            recordHabitEngine.record(usernameEntry.get(), passwordEntry.get(), graphEntry.get(), monthEntry.get(), dayEntry.get(), yearEntry.get(), quantityEntry.get())
        recordButton = Button(window, text="Record", font=("Arial", 12), bg=THEMECOLOR, fg=FOREGROUND, command=recordfunct)
        recordButton.grid(row=11, column=1, pady=10)